...
API requirements for Predictor module:
    1) Needs to be a pytorch module with 'forward()' function protocol:
        ...
        :param heatmap: (Tensor) Gaze heatmap (shape=[batch_size x num_frames x height x width])
...
import src.models.heatmap_predictor as vit_heatmap_pred
...
def init_module(
    ...
    **kwargs,
    ...
    # ----------------------------------------------------------------------- #
    # Initialize Predictor with Heatmap Support
    # ----------------------------------------------------------------------- #
    prd_kwargs = model_kwargs["predictor"]
    prd_ckp_key = prd_kwargs.get("checkpoint_key")
    prd_model_name = prd_kwargs.get("model_name", "vit_heatmap_predictor")

    predictor = vit_heatmap_pred.__dict__[prd_model_name](
        img_size=resolution,
    ...
    if prd_ckp_key in checkpoint:
        ...
        logger.info("No pretrained predictor weights found, using random initialization")
    ...
    # Build Wrapper
    # ----------------------------------------------------------------------- #
    model = AnticipativeWrapperHeatmap(
        encoder=encoder,
        predictor=predictor,
...
class AnticipativeWrapperHeatmap(torch.nn.Module):
    """Use predictor with heatmap for inference"""
    ...
    def forward(self, x, anticipation_times, heatmap=None):
        """
        :param x: (Tensor) video of shape [B, C, T, H, W]
        :param anticipation_times: (Tensor) [B] seconds into the future to predict for each sample in batch
        :param heatmap: (Tensor) [B, T, H, W] or [B, T, 1, H, W] gaze heatmap (optional)
        """
        x = self.encoder(x)
        ...
        # The heatmap should correspond to the context frames
        # For now, we'll pass the heatmap directly to the predictor
        # The predictor will handle the alignment
        # Note: heatmap should be aligned with the context frames, not future frames
        
        for _ in range(self.num_steps):
            # Pass heatmap to predictor if available
            # The predictor expects heatmap for context frames
            if heatmap is not None:
                x_pred = self.predictor(x, heatmap=heatmap)
            else:
                x_pred = self.predictor(x, heatmap=None)
