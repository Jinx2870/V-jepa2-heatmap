...
def main(args_eval, resume_preempt=False):
    ...
    # -- WANDB
    use_wandb = args_eval.get("use_wandb", False)
    wandb_project = args_eval.get("wandb_project", "vjepa-action-anticipation")
    wandb_name = args_eval.get("wandb_name", eval_tag)
    ...
    if dataset in ["COIN_anticipation", "EGO4D_KEYSTEP"]:
    ...
    if rank == 0 and use_wandb:
        wandb_config = {
            ...
            "num_heads": num_heads,
            "train_anticipation_time_sec": train_anticipation_time_sec,
            "val_anticipation_time_sec": val_anticipation_time_sec,
        }
    ...
    # Wrap with DDP only if distributed process group is initialized
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        # -- extract epoch-level losses for wandb (if available)
        train_loss_epoch = None
        val_loss_epoch = None
        if train_metrics is not None:
            try:
                train_loss_epoch = float(train_metrics["action"].get("loss", None))
            except Exception:
                train_loss_epoch = None
        try:
            val_loss_epoch = float(val_metrics["action"].get("loss", None))
        except Exception:
            val_loss_epoch = None
            ...
            if rank == 0:
                ...
                if wandb_logger:
                    metrics = {
                        ...
                        "train/verb_accuracy": train_metrics["verb"]["accuracy"],
                        "train/noun_accuracy": train_metrics["noun"]["accuracy"],
...
def train_one_epoch(
    ...
    # epoch-level loss accumulation (for wandb train/loss_epoch)
    epoch_loss_sum = 0.0
    epoch_loss_count = 0
    ...
    for itr in range(ipe):
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16, enabled=use_bfloat16):
            ...
            if action_is_verb_noun:
                ...
                # For action-only datasets like EGO4D_KEYSTEP, the dataloader already
                # provides class indices in udata[1]; use them directly.
                action_labels = udata[1].to(device)
        ...
        # accumulate epoch-level loss (average over classifiers, then over iters)
        iter_loss_value = None
        try:
            if action_is_verb_noun:
                _al = sum([float(l.detach().cpu()) for l in action_loss]) / len(action_loss)
                _vl = sum([float(l.detach().cpu()) for l in verb_loss]) / len(verb_loss)
                _nl = sum([float(l.detach().cpu()) for l in noun_loss]) / len(noun_loss)
                iter_loss_value = _al + _vl + _nl
            else:
                iter_loss_value = sum([float(l.detach().cpu()) for l in loss]) / len(loss)
        except Exception:
            iter_loss_value = None
        if iter_loss_value is not None:
            epoch_loss_sum += iter_loss_value
            epoch_loss_count += 1
            ...
            if action_is_verb_noun:
                ...
                if wandb_logger is not None and epoch is not None:
                    try:
                        ...
                        try:
                            ...
                            pass
                        payload = {
                            "train/iter_action_accuracy": max([a["accuracy"] for a in action_metrics]),
                            "train/iter_action_recall": max([a["recall"] for a in action_metrics]),
                            "train/iter_verb_accuracy": max([v["accuracy"] for v in verb_metrics]),
                            "train/iter_verb_recall": max([v["recall"] for v in verb_metrics]),
                            "train/iter_noun_accuracy": max([n["accuracy"] for n in noun_metrics]),
                            "train/iter_noun_recall": max([n["recall"] for n in noun_metrics]),
                        }
                        if _al is not None and _vl is not None and _nl is not None:
                            payload.update(
                                {
                                    "train/iter_loss": _al + _vl + _nl,
                                    "train/iter_loss_action": _al,
                                    "train/iter_loss_verb": _vl,
                                    "train/iter_loss_noun": _nl,
                                }
                            )
                ...
                if wandb_logger is not None and epoch is not None:
                    try:
                        ...
                        _tl = None
                        try:
                            # loss is a list of tensors across classifiers
                            _tl = sum([float(l.detach().cpu()) for l in loss]) / len(loss)
                        except Exception:
                            pass
                        payload = {
                            "train/iter_action_accuracy": max([a["accuracy"] for a in action_metrics]),
                        ...
                        if _tl is not None:
                            payload.update({"train/iter_loss": _tl})
                        wandb_logger.log(payload, step=step)
                    except Exception:
                        pass
    ...
    if epoch_loss_count > 0:
        train_loss_epoch = epoch_loss_sum / epoch_loss_count
    ret = dict(
        action=dict(
    ...
    if train_loss_epoch is not None:
        ret["action"]["loss"] = train_loss_epoch
    if action_is_verb_noun:
        ret.update(
            dict(
                verb=dict(
...
def validate(
    ...
    # epoch-level val loss accumulation (for wandb val/loss_epoch)
    epoch_loss_sum = 0.0
    epoch_loss_count = 0
    ...
    for itr in range(ipe):
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16, enabled=use_bfloat16):
            ...
            if action_is_verb_noun:
                ...
                # For action-only datasets like EGO4D_KEYSTEP, use provided class indices.
                action_labels = udata[1].to(device)
            ...
            # accumulate epoch-level val loss
            try:
    ...
    if val_loss_epoch is not None:
        ret["action"]["loss"] = val_loss_epoch
    if action_is_verb_noun:
        ret.update(
            dict(
                verb=dict(
